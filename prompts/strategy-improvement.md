...
> **触发时机**：Step 7 结论卡片输出完毕后**自动触发，不可跳过**。无需用户说「结束」或「归档」，AI 必须主动执行。
> 归档流程：先收集用户反馈评分 → 立即执行归档 → 输出简报。只在反馈收集时与用户交互一次，其余全自动。

---

## 一、收集用户反馈（强制输出，不可省略）

> 结论卡片输出后，AI 必须立即输出以下反馈收集格式，等待用户选择后再执行归档。
...
> 用户回复后立即执行归档（第二节），不再询问。若用户 60 秒内无回复，默认按 D 处理并继续归档。
...
| 清理临时文件 | 归档完成后按 `guards/temp-files.yaml` 清理规则自动执行；已标记 `.exported` 的文件跳过；清理结果附在归档简报末尾，不单独输出 |
